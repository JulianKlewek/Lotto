...
interface ResultAnnouncerSwagger {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully returned result for given ticketId",
                    content = {@Content(schema = @Schema(implementation = AnnouncerResponseDto.class),
                            mediaType = "application/json")})
    })
    ResponseEntity<AnnouncerResponseDto> getTicketResult(@PathVariable @Parameter(name = "ticketId",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully returned results for given draw date",
                    content = {@Content(schema = @Schema(implementation = AnnouncerWinningResultsResponseDto.class),
                            mediaType = "application/json")}),
            @ApiResponse(responseCode = "404", description = "Response if could not find winning numbers for given date",
                    content = {@Content(schema = @Schema(implementation = ApiError.class),
                            mediaType = "application/json")})
    })
    ResponseEntity<AnnouncerWinningResultsResponseDto> lotteryResultsForDate(@PathVariable @Parameter(name = "drawDate",
