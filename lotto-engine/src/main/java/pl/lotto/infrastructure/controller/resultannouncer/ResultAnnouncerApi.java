...
interface ResultAnnouncerApi {
    ...
    ResponseEntity<ResultResponse> getTicketResult(@PathVariable @Parameter(name = "ticketId",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully returned results for given draw date",
                    content = {@Content(schema = @Schema(implementation = WinningResultsResponse.class),
                            mediaType = "application/json")}),
            @ApiResponse(responseCode = "404", description = "Response if could not find winning numbers for given date",
                    content = {@Content(schema = @Schema(implementation = ApiError.class),
                            mediaType = "application/json")})
    })
    ResponseEntity<WinningResultsResponse> lotteryResultsForDate(@PathVariable @Parameter(name = "drawDate",
